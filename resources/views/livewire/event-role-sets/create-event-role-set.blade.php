<div class="rounded-lg border border-neutral-800 p-4 space-y-4">
    <div>
        <label class="block text-sm text-neutral-400 mb-1">Role set name</label>
    ...
    <div>
        <label class="block text-sm text-neutral-400 mb-1">Roles</label>
        <div class="space-y-2">
            @foreach ($roles as $index => $role)
                <div class="flex gap-2 items-start">
                    ...
                    <select wire:model="roles.{{ $index }}.capacity_mode" class="rounded bg-neutral-900 border border-neutral-700 px-2 py-2 text-sm">
                    ...
                    @if (($role['capacity_mode'] ?? 'uncapped') !== 'uncapped')
                        <input type="number" min="1" wire:model="roles.{{ $index }}.capacity" placeholder="Cap"
                               class="w-20 rounded bg-neutral-900 border border-neutral-700 px-2 py-2 text-sm">
                    @endif
                    @if (count($roles) > 1)
                        <button type="button" wire:click="removeRoleRow({{ $index }})" class="text-neutral-500 hover:text-red-400 px-2">&times;</button>
                    @endif
                </div>
            @endforeach
        </div>
        ...
        <button type="button" wire:click="addRoleRow" class="mt-2 text-xs text-indigo-400 hover:text-indigo-300">+ Add role</button>
    </div>
